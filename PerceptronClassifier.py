...
PATH_TO_DATA = "/home/tck/Documents/UMass/CS/CS 585 Fall 17/Homework/HW2/hw2/large_movie_review_dataset"
...
def predict_multiclass(bow, weights):
    ...
    # = theta vector dotproduct feature vector
...
def train(examples, stepsize=1, numpasses=10, do_averaging=False, devdata=None):
    ...
    for pass_iteration in range(numpasses):
        ...
        #bow = feature vector for one doc, goldlabel = true label for that doc
        for bow, goldlabel in examples:
            passes += 1
            predlabel = predict_multiclass(bow, weights)
            predvec = features_for_label(bow, predlabel)  # f(x,y*)    
            goodness = dict_subtract(bow, predvec)   # g = f(x,y)-f(x,y*)
            ...
            if predlabel != goldlabel:
                ...
                for w in goodness:
                    weights[w] += stepsize*(goodness[w])  # theta = old theta + rg
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc):
    ...
    X_LABEL = "Iterations Through Training Set"
    Y_LABEL = "Accuracy of Classifier"
    
    x = [i for i in range(1,len(train_acc)+1)]
    print("len x=", len(x))
    print("len train=", len(train_acc))
    print("len nb=", len(naive_bayes_acc))
    
    plt.plot(x, train_acc, label="Simple Perceptron Training Set")
    plt.plot(x, test_acc, label="Simple Perceptron Test Set")
    plt.plot(x, avg_test_acc, label="Averaged Perceptron Test Set")
    plt.plot(x, naive_bayes_acc, label="Naive Bayes Best")
    
    plt.xlabel(X_LABEL)
    plt.ylabel(Y_LABEL)
    plt.legend()
    plt.title("Accuracies of Several Binary Classifiers")
    plt.show()
...
if __name__=='__main__':
    ...
    nb = [.8364 for i in range(0,10)]
    plot_accuracy_vs_iteration(sol_dict['train_acc'], sol_dict['test_acc'], sol_dict['avg_test_acc'], nb)
